...
func (h PgsqlInitHandler) WriteConfig(ctx context.Context) error {
	...
	global.WM_CONFIG.System.DbType = "pgsql"
	global.WM_CONFIG.Pgsql = c
	global.WM_CONFIG.JWT.SigningKey = uuid.NewV4().String()
	cs := utils.StructToMap(global.WM_CONFIG)
	for k, v := range cs {
		global.WM_VP.Set(k, v)
	}
	return global.WM_VP.WriteConfig()
}

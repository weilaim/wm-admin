...
const initOrderAuthority = system.InitOrderSystem + 1

type initAuthority struct{}

func init() {
	fmt.Println("init-----------authority------------")
...
func (i *initAuthority) InitializeData(ctx context.Context) (context.Context, error) {
	fmt.Println("------initializeData authority --------")
	db, ok := ctx.Value("db").(*gorm.DB)
	if !ok {
		return ctx, system.ErrMissingDBContext
	}
	...
	entities := []sysModel.SysAuthority{
		...
		{AuthorityId: 8881, AuthorityName: "普通用户子用户", ParentId: utils.Pointer[uint](888), DefaultRouter: "dashboard"},
	}
...
func (i *initAuthority) DataInserted(ctx context.Context) bool {
	...
	if errors.Is(db.Where("authority_id = ?", "8881").First(&sysModel.SysAuthority{}).Error, gorm.ErrRecordNotFound) {
		// 判断是否存在数据
		return false
	}
